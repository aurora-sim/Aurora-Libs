...
public class LexerGenerate : TokensGen // used during LexerGenerate
{
	...
	string ActionTransform(string str)
	{
		...
		while (str!="") 
		{
			...
			else 
			{
				...
				White(str,ref a,str.Length);
				a++;
				ret.Append(str.Substring(0,a)).Append("yym");
				if (str[a]!=')') 
					ret.Append(",");
				str = str.Substring(a);
			}
		}
		return FixActions(ret.Append(str).ToString());
	}
	...
	public override SourceLineInfo sourceLineInfo(int pos) 
	{ 
		return new SourceLineInfo(m_LineManager,pos); 
	}
